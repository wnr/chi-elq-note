...
\section{Implementation}
  ...
  \subsection{Element Resize Detection}
    ...
    Even if some layout engines are unable to queue the repositing of scrollbars, it is still beneficial to batch process the algorithm since only pure layouts need to be performed (instead of having to recompute styles and synchronize the \gls{DOM} and \glspl{render tree} before each layout).
...
\section{Empirical evaluation}
  \subsection{Performance}
    ...
    %\subsection{Element resizing detection}\label{sec:eval-perf-erd}
      ...
      It achieves a 32-fold speedup compared to the object-based solution and a 13-fold speedup compared to the scroll-based solution when preparing 500 elements for resize detection.
      The \gls{ELQ} solution also scales better, as more clearly shown in figure~\ref{fig:erd-solutions-regressed} that includes polynomial regression graphs for all three solutions.
      Both scroll-based solutions have the same memory footprint (i.e., too low for reliable measurements).
      \begin{figure*}[h]
        \tiny
      ...
      See table~\ref{table:erd-layout-engines} for the performance of the object-based and scroll-based solutions operating on 100 elements in different \glspl{browser}.
      ...
      In Firefox, the object-based solution detects resize events 2-fold faster than the \gls{ELQ} scroll-based solution when operating on 100 elements (still, 100 ms for detecting resize events is acceptable).
      ...
      \begin{table}[ht]\center
        ...
        \end{tabular}
        \caption{Performance of the object-based and \gls{ELQ} scroll-based solutions in different \glspl{layout engine} when operating on 100 elements.}
...
\section{Discussion}
  ...
  \subsection{Limitations}
    ...
    % Since they are event based, \glspl{layout engine} only need to perform extra work when injecting the elements to the target elements and when the target elements actually resize.
    % Event-based resize detection also implies minimal delay between the actual resize event and the detection.
    % By positioning the injected elements \code{absolute} with widths and heights set to \code{100\%} and the \code{visibility} set to \code{hidden}, the injected elements do not affect the visual representation of the \gls{document}.
    % However, injecting elements into target elements has some implications:
    % \begin{itemize}
    %   \item \textbf{\gls{CSS} selectors may break}:
    %     Since target elements get an extra child, \gls{CSS} selectors may behave differently.
    %     For instance, the selector \code{\#target~*} also matches the injected elements, which may result in them being styled in an undesired way.
    ...
    %     Absolute positioned elements cannot be styled relative to \code{static} positioned elements and are therefore moved up to the first non-static positioned ancestor in the render tree \cite{w3c_visuren}.
    %     Since the injected element must be a child of the target \gls{element} (and not moved upwards in the render tree), the target \gls{element} cannot be positioned \code{static} (i.e., the default positioning for many elements).
    %     Fortunately, elements positioned \code{relative} behave exactly like \code{static}, given they do not have any styles applicable to relative elements (such as \code{top} or \code{bottom}).
    ...
    %     This way target elements can be positioned \code{relative} with the special style properties removed, to obtain the same visual representation as being positioned \code{static}.
    % \end{itemize}
