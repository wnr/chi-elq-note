...
\tocauthor{Lucas Wiener, Tomas Ekholm, and Philipp Haller}
\maketitle
...
\section{Overview of \elq{}}\label{sec:elq}\label{sec:elq-api}
  ...
  \paragraph{The default breakpoints API.}
    ...
    \elq{} can either be required as a module (by the CommonJS or AMD syntax), or it can be included in an HTML \code{script} tag which then exposes a global constructor \code{Elq}.
    The following is an example of how to create an \elq{} instance and activate elements:
    ...
    \paragraph{Nested modules.}
      ...
      \noindent
      In this example, the paragraph elements (\verb|<p elq elq-mirror>...</p>|) always have the same element breakpoint classes as the parent \code{elq-breakpoints} elements.
...
\section{Extensions via plugins}\label{sec:plugins}
  ...
  \noindent
  All of the methods are invoked when registered with an \elq{} instance.
  The \code{getName} and \code{getVersion} methods tell the name and version of the plugin.
  The \code{isCompatible} tells if the plugin is compatible with the \elq{} instance that it is registered with.
  In the \code{make} method the plugin may initialize itself with the \elq{} instance and return an object that defines the plugin API accessible by \elq{} and other plugins.
...
\section{Empirical evaluation}\label{sec:eval}
  \subsection{Performance}\label{sec:eval-perf}
    Only the performance of the element resize detection system has been evaluated.
    This due to the fact that detecting element resize events entails all the significant performance penalties of \elq{}.
  ...
  \subsection{Case studies}
    ...
    In addition to the Bootstrap case study, we have been gathering experience with the application of \elq{} in large financial applications developed at EVRY AB, Sweden.
...
\section{Discussion}\label{sec:discussion}
  \paragraph{Limitations.}
    ...
    It should be noted that all limitations described only affect the elements that uses the element queries functionality.
  ...
  \paragraph{Standardization.}
    ...
    Atkins argues that a full implementation that avoids the double layout issue is unlikely to be implemented, and therefore it might be wiser to pursue sub-standards which aid third-party solutions instead.
