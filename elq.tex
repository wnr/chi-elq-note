...
\section{Introduction}
  ...
  This can be achieved with the theoretical feature \emph{element/container queries} that enables conditional style rules by an arbitrary element size.
...
\section{Examples of Broken RWD Today}
  ...
  To overcome this problem we must change the application so that widgets always have the same size. This implies that the size of the module and the media 
  query breakpoints are coupled/intertwined, i.e. they are proportional to each other.
...
The problem now is that we have removed the reusability of the weather module, since it requires the specific width that is correctly proportional to the media query breakpoints. 
  ...
  The latter requirement are impossible without element queries. 
