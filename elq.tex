...
%
\authorrunning{Modular Responsive Web Design using\\ Element Queries}
% (feature abused for this document to repeat the title also on left hand pages)

% the affiliations are given next; don't give your e-mail address
% unless you accept that it will be published
\institute{KTH Royal Institute of Technology and EVRY AB
\mailsa\\}
...
\section{Introduction}
  ...
  Section~\ref{sec:imp} provides an overview of \elq{}'s implementation.
...
\section{Overview of \elq{}}\label{sec:elq}\label{sec:elq-api}
  ...
  \paragraph{The default breakpoints API.}
    ...
    \begin{lstlisting}[gobble=6,caption={}]]
      var elq = Elq();         // Create an ELQ instance
      elq.use(myPlugin);       // Registering a plugin
      elq.activate(elements);  // Activate given elements
    \end{lstlisting}
...
\section{Implementation}\label{sec:imp}
  \subsection{Element Resize Detection}\label{sec:imp_erd}
    ...
    We use a scroll-based approach in order to detect element resize events.
    It injects multiple overflowing elements that listen to scroll events.
    The overflowing elements are styled so that \code{scroll} events are emitted when the target element is resized.
...
\section{Empirical evaluation}\label{sec:eval}
  ...
  \subsection{Case studies}
    ...
    In order to answer the questions, we have adapted the popular Bootstrap framework (version 3.3.2) to use element queries instead of media queries.
...
\section{Appendix}
  ...
  \subsection{Implementing element resize detection}
