...
\section{Element Queries}
  ...
  \subsection{JavaScript implementations}
    \begin{itemize}
      ...
      \item Why must it be a client side JS library?
      \item Note drawbacks (but only drawbacks for added functionality!).
      \item Mention that there is related work.
    \end{itemize}
    Implementing element queries as a third-party solution must be in the form of a client side library to be included in applications that makes sure all element queries are updated when necessary.
    One could argue that a solution does not need to be executed at client side, but instead generate media queries server side for all modules with respect to the current application layout.
